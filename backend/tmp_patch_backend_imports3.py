...
replacements = {
    r'\bfrom\s+ai\.': 'from ai.',
    r'\bfrom\s+auth\.': 'from auth.',
    r'\bfrom\s+api\.': 'from backend.api.',
    r'\bfrom\s+database\.': 'from database.',
    r'\bfrom\s+encryption\.': 'from encryption.',
    r'\bfrom\s+ocr\.': 'from backend.ocr.',
    r'\bfrom\s+security\.': 'from backend.security.',
    r'\bfrom\s+storage\.': 'from storage.',
    r'\bfrom\s+tasks\.': 'from tasks.',
